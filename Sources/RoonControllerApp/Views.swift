            ...
            Section("Zones") {
                if model.zones.isEmpty {
                    ...
                    ForEach(model.zones) { zone in
                        ZoneRow(zone: zone, isSelected: zone.zoneID == model.selectedZoneID) {
                            model.selectedZoneID = zone.zoneID
                            model.refreshBrowse()
                        }
                    }
                }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if model.selectedHierarchy != .search,
               let promptItem = model.browsePage?.items.first(where: { $0.inputPrompt != nil }) {
                SearchPromptRow(
                    item: promptItem,
                    text: $promptText
                ) {
                        ...
                        LazyVGrid(
                            ...
                            ForEach(page.items) { item in
                                BrowserGridCard(item: item)
                                    .onAppear {
                                        model.loadNextBrowsePageIfNeeded(currentItem: item)
                                    }
                            }
                        }
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 16) {
                if let zone = model.selectedZone {
                    HStack(spacing: 14) {
                        ArtworkView(
                            imageKey: zone.nowPlaying?.imageKey,
                            title: zone.nowPlaying?.title ?? zone.displayName,
                            size: CGSize(width: 52, height: 52)
                        )

                        VStack(alignment: .leading, spacing: 3) {
                            Text(zone.nowPlaying?.title ?? zone.displayName)
                                .font(.system(size: 13, weight: .semibold))
                                .lineLimit(1)
                            Text(zone.nowPlaying?.subtitle ?? zone.displayName)
                                .font(.system(size: 12))
                                .foregroundStyle(.secondary)
                                .lineLimit(1)
                            Text(zone.state.capitalized)
                                .font(.caption)
                    ...
                    Label("No Zone Selected", systemImage: "airplayaudio")
                        .foregroundStyle(.secondary)
                ...
                HStack(spacing: 14) {
                    ...
                    MiniPlayerButton(systemName: "stop.fill", enabled: model.selectedZone?.capabilities.canStop == true) {
        ...
        VStack(alignment: .leading, spacing: 12) {
            Button {
                ...
                ArtworkView(
                    imageKey: item.imageKey,
                    title: item.title,
                    size: CGSize(width: 172, height: 172)
                )
            }
            .buttonStyle(.plain)
            .frame(maxWidth: .infinity, alignment: .leading)
